...
template<typename Scalar> struct scalar_tanh_op {
    ...
    CNN_DEVICE_FUNC inline const Scalar operator() (const Scalar& a) const {
        using std::tanh;
        return tanh(a);
    }
    template <typename Packet>
    CNN_DEVICE_FUNC inline Packet packetOp(const Packet& a) const {
        return Eigen::internal::ptanh(a);
    }
};
}
...
template<typename Scalar> struct scalar_tanh_backward_op {
    ...
    CNN_DEVICE_FUNC EIGEN_STRONG_INLINE const Scalar operator() (const Scalar& t, const Scalar& d) const {
        return (1 - t * t) * d;
    }
    template<typename Packet>
