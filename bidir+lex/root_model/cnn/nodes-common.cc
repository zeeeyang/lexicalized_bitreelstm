...
Dim SparsemaxLoss::dim_forward(const vector<Dim>& xs) const {
    if (xs.size() != 1 || !LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in SparsemaxLoss: " << xs;
...
Dim Sparsemax::dim_forward(const vector<Dim>& xs) const {
    if (xs.size() != 1 || !LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in Sparsemax: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim KMHNGram::dim_forward(const vector<Dim>& xs) const {
    ...
    if (new_cols < 1) {
        ostringstream s;
        s << "Bad input dimensions in KMHNGram: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim InnerProduct3D_1D::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].ndims() != 3 ||
        ...
        ostringstream s;
        s << "Bad input dimensions in InnerProduct3D_1D: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim InnerProduct3D_1D_1D::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].ndims() != 3 ||
        ...
        // TODO fix add check
        ostringstream s;
        s << "Bad input dimensions in InnerProduct3D_1D_1D: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim LogSumExp::dim_forward(const vector<Dim>& xs) const {
    ...
    for (unsigned i = 1; i < xs.size(); ++i) {
        if (d.single_batch() != xs[i].truncate().single_batch()) {
            ostringstream s;
            s << "Mismatched input dimensions in LogSumExp: " << xs;
            throw std::invalid_argument(s.str());
        }
...
Dim Sum::dim_forward(const vector<Dim>& xs) const {
    ...
    for (unsigned i = 1; i < xs.size(); ++i) {
        if (d.single_batch() != xs[i].truncate().single_batch()) {
            ostringstream s;
            s << "Mismatched input dimensions in Sum: " << xs;
            throw std::invalid_argument(s.str());
        }
        d.bd = max(xs[i].bd, d.bd);
    }
...
Dim Average::dim_forward(const vector<Dim>& xs) const {
    ...
    for (unsigned i = 1; i < xs.size(); ++i) {
        if (xs[0].single_batch() != xs[1].single_batch()) {
            ostringstream s;
            s << "Mismatched input dimensions in Average: " << xs;
            throw std::invalid_argument(s.str());
        }
        d.bd = max(xs[i].bd, d.bd);
    }
...
Dim Concatenate::dim_forward(const vector<Dim>& xs) const {
    ...
    for (auto c : xs) {
        ...
        if (dr.single_batch() != c.single_batch()) {
            ostringstream s;
            s << "Bad input dimensions in Concatenate: " << xs;
            throw std::invalid_argument(s.str());
        }
...
Dim ConcatenateColumns::dim_forward(const vector<Dim>& xs) const {
    ...
    for (auto& d : xs) {
        if (d[0] != rows) {
            ostringstream s;
            s << "Bad input dimensions in ConcatenateColumns: " << xs;
            throw std::invalid_argument(s.str());
        }
        new_cols += d[1];
        bd = max(bd, d.bd);
...
Dim PairwiseRankLoss::dim_forward(const vector<Dim>& xs) const {
    if (xs.size() != 2 ||
        ...
        ostringstream s;
        s << "Bad input dimensions in PairwiseRankLoss: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0].bd >= xs[1].bd ? xs[0] : xs[1];
}
...
Dim Hinge::dim_forward(const vector<Dim>& xs) const {
    if (xs.size() != 1 || !LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in Hinge: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim Softmax::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in Softmax: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim SoftSign::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in SoftSign: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim PickNegLogSoftmax::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in PickNegLogSoftmax: " << xs;
        throw std::invalid_argument(s.str());
    }
    return Dim({1}, xs[0].bd);
}
...
Dim LogSoftmax::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in LogSoftmax: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim RestrictedLogSoftmax::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in RestrictedLogSoftmax: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim PickElement::dim_forward(const vector<Dim>& xs) const {
    ...
    if (!LooksLikeVector(xs[0])) {
        ostringstream s;
        s << "Bad input dimensions in PickElement: " << xs;
        throw std::invalid_argument(s.str());
    }
    return Dim({1}, xs[0].bd);
}
...
Dim MatrixMultiply::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].cols() != xs[1].rows()) {
        ostringstream s;
        s << "Mismatched input dimensions in MatrixMultiply: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim CwiseMultiply::dim_forward(const vector<Dim>& xs) const {
    ...
    if (d.single_batch() != xs[1].truncate().single_batch()) {
        ostringstream s;
        s << "Mismatched input dimensions in CwiseMultiply: " << xs;
        throw std::invalid_argument(s.str());
    }
...
Dim Pow::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[1].truncate().single_batch().size() != 1) {
        ostringstream s;
        s << "Bad input dimensions in Pow: " << xs;
        throw std::invalid_argument(s.str());
    }
    return d;
}
...
Dim CwiseQuotient::dim_forward(const vector<Dim>& xs) const {
    ...
    if (d.single_batch() != xs[1].truncate().single_batch()) {
        ostringstream s;
        s << "Bad input dimensions in CwiseQuotient: " << xs;
        throw std::invalid_argument(s.str());
    }
    d.bd = max(xs[1].bd, d.bd);
    return d;
...
Dim AffineTransform::dim_forward(const vector<Dim>& xs) const {
    if ((xs.size() - 1) % 2 != 0) {
        ostringstream s;
        s << "Bad number of inputs in AffineTransform: " << xs;
        throw std::invalid_argument(s.str());
    }
    Dim d = xs[0];
    for (unsigned i = 1; i < xs.size(); i += 2) {
        if (xs[i].cols() != xs[i+1].rows() ||
            ...
            ostringstream s;
            s << "Bad dimensions for AffineTransform: " << xs;
            throw std::invalid_argument(s.str());
        }
...
Dim HuberDistance::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].single_batch() != xs[1].single_batch()) {
        ostringstream s;
        s << "Mismatched input dimensions in HuberDistance: " << xs;
        throw std::invalid_argument(s.str());
    }
    return Dim({1}, max(xs[0].bd, xs[1].bd));
}
...
Dim L1Distance::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].single_batch() != xs[1].single_batch()) {
        ostringstream s;
        s << "Mismatched input dimensions in L1Distance: " << xs;
        throw std::invalid_argument(s.str());
    }
    return Dim({1}, max(xs[0].bd, xs[1].bd));
}
...
Dim PoissonRegressionLoss::dim_forward(const vector<Dim>& xs) const {
    if (xs.size() != 1 || xs[0].size() != 1) {
        ostringstream s;
        s << "Bad input dimensions in PoissonRegressionLoss: " << xs;
        throw std::invalid_argument(s.str());
    }
    return xs[0];
}
...
Dim SquaredEuclideanDistance::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].single_batch() != xs[1].single_batch()) {
        ostringstream s;
        s << "Bad input dimensions in SquaredEuclideanDistance: " << xs;
        throw std::invalid_argument(s.str());
    }
    return Dim({1}, max(xs[0].bd, xs[1].bd));
}
...
Dim BinaryLogLoss::dim_forward(const vector<Dim>& xs) const {
    ...
    if (xs[0].rows() != 2 && xs[0].ndims() != 1) {
        ostringstream s;
        s << "Bad input dimensions in BinaryLogLoss: " << xs;
        throw std::invalid_argument(s.str());
    }
