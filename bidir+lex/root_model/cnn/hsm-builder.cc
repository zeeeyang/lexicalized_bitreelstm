...
const vector<unsigned>& Cluster::get_path() const {
    return path;
}
unsigned Cluster::get_index(unsigned word) const {
    return word2ind.find(word)->second;
}
unsigned Cluster::get_word(unsigned index) const {
...
inline bool is_ws(char x) {
    return (x == ' ' || x == '\t');
}
inline bool not_ws(char x) {
    return (x != ' ' && x != '\t');
}
...
Cluster* HierarchicalSoftmaxBuilder::ReadClusterFile(const std::string& cluster_file, Dict* word_dict) {
    ...
    while(getline(in, line)) {
        ...
        while (startp < len) {
            ...
            endp = startp;
            while (not_ws(line[endp]) && endp < len) {
                ++endp;
            }
        ...
        while (is_ws(line[startw]) && startw < len) {
            ++startw;
        }
        unsigned endw = startw;
        while (not_ws(line[endw]) && endw < len) {
            ++endw;
        }
        assert(endp > startp);
        assert(startw > endp);
