                                  ...
                                  bool verbose)
{
    ...
    softmax(cg, tree, *tree.root, loss, total, rcor, pcor, eval, verbose);
    return sum(loss);
}

void BiTreeBuilder::softmax(ComputationGraph& cg,
                            BiTree& bitree,
                            struct BiTreeNode& treeNode,
                            vector<Expression>& loss,
                            int& total,
                            int& rcor,
                            int& pcor,
                            ...
                            bool verbose)
{
    ...
    Expression i_r_t = rectify(affine_transform({vars[TB], vars[BU2T], treeNode.bu_hi, vars[TD2T], treeNode.output_td_hi, vars[OUTPUTTD2T],  treeNode.td_hi}));  //using node average
    ...
    if(eval&&loss.empty())
    {
        ...
        if(verbose) {
            ...
            /*if(besti == tree.label)
              ...
              cerr<<"0" << endl;*/
            /*vector<string> words;
            bitree.printWords(words, treeNode);
            copy(words.begin(), words.end(), ostream_iterator<string>(cerr, " "));
            cerr<<" ||| ";*/
            cerr<<besti<<endl; // output label
            //cerr<<endl;
        }
        if(besti == treeNode.label ) {
            if(loss.empty())
                rcor++; //root
            pcor++;
        }
    ...
    Expression nll = pickneglogsoftmax(o_t, treeNode.label);
    loss.push_back(nll);
    if(eval) //root-level only
        return;
    for(auto& child: treeNode.children)
        softmax(cg, bitree, *child, loss, total, rcor, pcor, eval, verbose);
}
